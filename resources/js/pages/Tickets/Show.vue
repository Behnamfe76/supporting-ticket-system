    ...
    if (replies.length) {
        replies.forEach((item: any, index: number) => {
            ...
            message.items.push({
                ...
                icon: isCurrentUser ? (isUserRole ? 'UserRound' : 'UserRoundCheck') : (item.author.roles && item.author.roles.includes('user') ? 'UserRound' : 'UserRoundCheck')
            });
        });
    }
});
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="flex h-full flex-col">
                <div class="flex min-h-0 flex-1 overflow-hidden">
                                ...
                                <div class="flex h-16 flex-col justify-center">
                                    <div class="px-4 sm:px-6 lg:px-8">
                                        <div class="flex justify-between py-3">
                                            ...
                                            <div>
                                                <div
                                                    class="isolate inline-flex rounded-md shadow-xs sm:space-x-3 sm:shadow-none">
                                                    <span class="inline-flex sm:shadow-xs">
                                                        <button @click="replyModal = true" type="button"
                                                            ...
                                                            <ArrowUturnLeftIcon class="-ml-0.5 size-5 text-gray-400"
                                                                aria-hidden="true" />
                                                            Reply
                                                        </button>
                                                    </span>
                            ...
                            <div class="min-h-0 flex-1 overflow-y-auto">
                                <div class="bg-white pt-5 pb-6 shadow-sm">
                                    <div class="px-4 sm:flex sm:items-baseline sm:justify-between sm:px-6 lg:px-8">
                                        ...
                                        <div
                                            class="mt-4 flex items-center justify-between sm:mt-0 sm:ml-6 sm:shrink-0 sm:justify-start">
                                            <span :class="generateStatusStyles(message.status)"
                    ...
                    <!-- reply modal -->
                    <Dialog v-model:visible="replyModal" maximizable modal header="Reply Ticket"
